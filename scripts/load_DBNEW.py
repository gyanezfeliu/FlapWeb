...
# Función que runea el código, es llamada desde el formulario al submitir los campos
def run(*args):
    ...
    # Cargo Metadata que está almacenada como un string de JSON en la BD, lo recupero 
    # y lo estructuro en un DataFrame
    ...
    #Si el usuario no me da el nombre del experimento, le pongo el nombre del archivo de datos
    ...
    # Cargo el excel subido por el usuario
    ...
    # Arreglo el excel para dejar casi puros datos
    lista_rows = find_index(ws['A'], medidas)
    ws.delete_rows(0, lista_rows[0][1] - 1)
    lista_rows2 = find_index(ws['A'], medidas)


    # Aquí empiezo el ciclo de armar el array con las series de tiempo que serán almacenadas 
    # en la base de datos. También extraigo el tiempo para cada dato y su nombre.


    # array consolidado que tiene los nombres para cada medición
    names = np.array([], dtype='str')
    # array consolidado que tiene las mediiones
    data = np.array([])
    # array consolidado que tiene los tiempos de las mediciones
    times = np.array([])

    # datos que fueron extraído del excel del lector de placas
    raw_data = np.array(list(ws.values))

    # cantidad de datos de las series de tiempo
    dim = lista_rows2[1][1] - lista_rows2[0][1] - 4

    # Paso los datos a una matriz, nombres a un vector y tiempos a un vector.
    ...
    # TO DO: HACER UN IF DE VERIFICACION DE QUE LAS DIMENSIONES CALZAN
